...
function isUsernameTaken(username) {
    ...
    return lobbyUsers.some(user => user.username.toLowerCase() === username.toLowerCase());
}

function sendRegistrationSuccess(ws) {
    // Send a success message to the user
    ws.send(JSON.stringify({ type: 'success', message: 'Registration successful' }));
}
...
function broadcastStateChange(username, state) {
    ...
    const message = JSON.stringify({ type: 'stateChange', username, state });
